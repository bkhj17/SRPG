...
{
	material = new Material(RECTANGLE_SHADER);
	hullShader = Shader::AddHS(RECTANGLE_SHADER);
	domainShader = Shader::AddDS(RECTANGLE_SHADER);

	mesh = new Mesh<Vertex>();
	vector<Vertex>& vertices = mesh->GetVertices();
	/*
	vertices.emplace_back(+0.0f, +0.9f, 0.0f);
...
void TessellationScene::GUIRender()
{
	...
	ImGui::DragFloat("Inside0", &insideBuffer->Get()[0]);
	ImGui::DragFloat("Inside1", &insideBuffer->Get()[1]);
}
