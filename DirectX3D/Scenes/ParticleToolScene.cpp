...
void ParticleToolScene::UpdatePhysical()
{
	...
	for (UINT i = 0; i < data.count; i++) {
		...
		particleInfos[i].transform.Scale() = Lerp(particleInfos[i].startScale, particleInfos[i].endScale, t);

		particleInfos[i].transform.UpdateWorld();
		instances[drawCount++].transform = XMMatrixTranspose(particleInfos[i].transform.GetWorld());
	}
...
void ParticleToolScene::Load(string file)
{
	...
	quad->GetMaterial()->SetDiffuseMap(Texture::Add(textureFile));
