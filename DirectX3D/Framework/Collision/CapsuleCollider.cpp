...
bool CapsuleCollider::IsBoxCollision(BoxCollider* collider)
{
	...
	Vector3 closestPointToBox = box.pos;
	...
	for (UINT i = 0; i < 3; i++) {
		...
		closestPointToBox += box.axis[i] * length * mult;
	}

	float distance = Distance(pointOnLine, closestPointToBox);
...
bool CapsuleCollider::IsCapsuleCollision(CapsuleCollider* collider)
{
	...
	Vector3 bA = GlobalPos() - bDirection * Height() * 0.5f;
	Vector3 bB = GlobalPos() + bDirection * Height() * 0.5f;
